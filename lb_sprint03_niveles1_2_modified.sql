-- Creo la tabla credit_card dentro de database transactions

-- Nivel 1
-- Ejercicio 1
-- Tengo que eliminar la tabla credit_card para tener los pantallazos que faltan
-- Para esto, antes tengo que eliminar la foreign_key que relaciona credit_card cpn transaction. este es el codigo:
	/*alter table transaction
	drop foreign key transaction_ibfk_2;*/

-- Luego elimino la tabla credit_card. Script abajo:
-- drop table credit_card

-- Vuelvo a crear la tabla credit_card
...
-- Intento modificar la tabla transaction para añadirle el id de la table credit_card como Foreign Key con este script
	ALTER TABLE transaction
	ADD FOREIGN KEY (credit_card_id) REFERENCES credit_card(id); 
 
 -- No me permite hacerlo. Da error 1452. Parece que hay registros de tabla trasaction que no estan en la tabla credit_card que he vuelto a crear
 -- Primero verifico si es esto es lo que está pasando.  Y si hay un registro que no tiene credit_card: es el CcU-9999
select distinct credit_card_id 
from `transaction` 
where credit_card_id not in (select id from credit_card);

-- Tengo que agregar este registro a la tabla cfredit_ card, para poder relacionarla nuevamente con la tabla transaction
-- Esto implica que tengo que adelantar lo que nos piden hacer en un ejercicio 3 del Nivel 1
    ...
    -- Ahora puedo modificar la tabla transaction para añadirle el id de la tabla credit_card como Foreign Key
	alter table transaction
	add foreign key (credit_card_id) references credit_card(id); 

-- Ejercicio 2
-- Modificar el numero de cuenta de la compañia (iban) con credit_card_id CcU-2938 a R323456312213576817699999

-- Este es el iban erroneo (antes del cambio):
select * from credit_card
where id = 'Ccu-2938';

-- Modifico el iban para que sea igual a R323456312213576817699999
update credit_card
set iban = 'R323456312213576817699999'
WHERE id = 'CcU-2938';

-- Este es el iban correcto tras el cambio
select * from credit_card
where id = 'Ccu-2938';

-- Ejercicio 3
-- En la tabla "transaction" tengo que ingresar un nuevo usuario con la info que me dan.
-- Primero tengo que registrar a la nueva compañia b-9999 en la tabla company
	insert into company (id)
	values ('b-9999');
 
 -- Luego registro la tarjeta de crédito en la tabla credit_card
	insert into credit_card (id)
    values ('CcU-9999');

-- Finalmente inserto la nueva transaccion que corresponde a la nueva company
...
-- Ejercicio 2
/* crees una vista llamada VistaMarketing que contenga la siguiente información: 
